<?php

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;
use App\Http\Controllers\Api\V1\Auth\AuthController;
use App\Http\Controllers\Api\V1\Auth\VerificationController;
use App\Http\Controllers\Api\V1\Auth\ForgotPasswordController;
use App\Http\Controllers\Api\V1\MediaController;
use App\Http\Controllers\Api\V1\EpreuveController;
use App\Http\Controllers\Api\V1\EnfantsController;
use App\Http\Controllers\Api\V1\MatiereController;
use App\Http\Controllers\Api\V1\ParentsController;
use App\Http\Controllers\Api\V1\PayementController;
use App\Http\Controllers\Api\V1\PosteController;
use App\Http\Controllers\Api\V1\RepetiteurController;
use App\Http\Controllers\Api\V1\RoleController;
use App\Http\Controllers\Api\V1\UserController;
use App\Http\Controllers\Api\V1\DemandeController;
use App\Http\Controllers\Api\V1\ClasseController;
use App\Http\Controllers\Api\V1\CommuneController;
use App\Http\Controllers\Api\V1\EcoleController;
use App\Http\Controllers\Api\V1\EvaluationsController;
use App\Http\Controllers\Api\V1\LibrairieController;
use App\Http\Controllers\Api\V1\MessageController;
use App\Http\Controllers\Api\V1\NotificationController;
use App\Http\Controllers\Api\V1\PubliciteController;
use App\Http\Controllers\Api\V1\TarificationController;
use App\Http\Controllers\Api\V1\RepetiteurMatiereClasseController;
use App\Models\Role;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

Route::middleware('json-response')->prefix('auth')->group(function () {
    // route to register new user for the platform
    Route::post("/register", [AuthController::class, 'register'])->name('api.register');
    // route to log the user if he has already sign up
    Route::post("/login", [AuthController::class, 'login'])->name('api.login');

    Route::post("/getUserInfo", [AuthController::class, 'getUserInfo'])->name('api.getUserInfo');
    // route to send reset link to email for password forgotten
    Route::post('/password/email', [ForgotPasswordController::class, 'sendResetLinkEmail']);
    // route to send reset password for password forgotten
    Route::post('/password/reset', [ForgotPasswordController::class, 'reset'])->name('passwords.reset');
    // route to resend the email verification when the link has expired
    Route::post('resend/', [VerificationController::class, 'resend'])->name('verification.resend');
    // route to verify email after clicking on the link on email
    Route::get('email/verify/{user}', [VerificationController::class, 'verify'])->name('verification.verify')
        ->middleware('signed');
});

// route pour demande
Route::get('/demandes' ,[DemandeController::class, 'index']);
Route::get('/demandes/{id}', [DemandeController::class, 'show']);




// route pour epreuves
Route::get('/epreuves' ,[EpreuveController::class, 'index']);
Route::get('/epreuves/{id}', [EpreuveController::class, 'show']);
Route::post('/epreuves', [EpreuveController::class, 'store']);
Route::put('/epreuves/{id}', [EpreuveController::class, 'update']);
Route::delete('/epreuves/{id}',[EpreuveController::class, 'destroy']);

// route pour enfants
Route::get('/enfants' ,[EnfantsController::class, 'index']);
Route::get('/enfants/{id}', [EnfantsController::class, 'show']);

//route pour matiere
Route::get('/matieres' ,[MatiereController::class, 'index']);
Route::get('/matieres/{id}', [MatiereController::class, 'show']);
Route::post('/matieres', [MatiereController::class, 'store']);
Route::put('/matieres/{id}', [MatiereController::class, 'update']);
Route::delete('/matieres/{id}',[MatiereController::class, 'destroy']);

// routes pour les classes
Route::get('/classes' ,[ClasseController::class, 'index']);
Route::get('/classes/{id}', [ClasseController::class, 'show']);
Route::post('/classes', [ClasseController::class, 'store']);
Route::put('/classes/{id}', [ClasseController::class, 'update']);
Route::delete('/classes/{id}',[ClasseController::class, 'destroy']);


//routes pour Tarification
Route::get('/tarifications' ,[TarificationController::class, 'index']);
Route::get('/tarifications/{id}', [TarificationController::class, 'show']);
Route::post('/tarifications', [TarificationController::class, 'store']);
Route::put('/tarifications/{id}', [TarificationController::class, 'update']);
Route::delete('/tarifications/{id}',[TarificationController::class, 'destroy']);



//routes pour Publicités
Route::get('/publicites' ,[PubliciteController::class, 'index']);
Route::get('/publicites/{id}', [PubliciteController::class, 'show']);
Route::post('/publicites', [PubliciteController::class, 'store']);
Route::put('/publicites/{id}', [PubliciteController::class, 'update']);
Route::delete('/publicites/{id}',[PubliciteController::class, 'destroy']);

//routes pour Ecole
Route::get('/ecoles' ,[EcoleController::class, 'index']);
Route::get('/ecoles/{id}', [EcoleController::class, 'show']);
Route::post('/ecoles', [EcoleController::class, 'store']);
Route::put('/ecoles/{id}', [EcoleController::class, 'update']);
Route::delete('/ecoles/{id}',[EcoleController::class, 'destroy']);

//routes pour Librairie
Route::get('/librairies' ,[LibrairieController::class, 'index']);
Route::get('/librairies/{id}', [LibrairieController::class, 'show']);
Route::post('/librairies', [LibrairieController::class, 'store']);
Route::put('/librairies/{id}', [LibrairieController::class, 'update']);
Route::delete('/librairies/{id}',[LibrairieController::class, 'destroy']);


//routes pour repetieur,pivot
Route::get('/repetiteurmcs' ,[RepetiteurMatiereClasseController::class, 'index']);
Route::get('/repetiteurmcs/{id}', [RepetiteurMatiereClasseController::class, 'show']);


//route pour payement
Route::get('/payements' ,[PayementController::class, 'index']);
Route::get('/payements/{id}', [PayementController::class, 'show']);


// route pour les User
Route::get('/users', [UserController::class, 'index']);
Route::get('/users/{id}', [UserController::class, 'show']);
Route::post('/users', [UserController::class, 'store']);
Route::put('/users/{id}', [UserController::class, 'update']);
Route::delete('/users/{id}', [UserController::class, 'destroy']);



// route pour les Message
Route::get('/messages', [MessageController::class, 'index']);
Route::get('/messages/{id}', [MessageController::class, 'show']);
Route::post('/messages', [MessageController::class, 'store']);
Route::put('/messages/{id}', [MessageController::class, 'update']);
Route::delete('/messages/{id}', [MessageController::class, 'destroy']);


// route pour les evaluation
Route::get('/evaluations', [EvaluationsController::class, 'index']);
Route::get('/evaluations/{id}', [EvaluationsController::class, 'show']);
Route::post('/evaluations', [EvaluationsController::class, 'store']);
Route::put('/evaluations/{id}', [EvaluationsController::class, 'update']);
Route::delete('/evaluations/{id}', [EvaluationsController::class, 'destroy']);

// route pour les Notifications
Route::get('/notifications', [NotificationController::class, 'index']);
Route::get('/notifications/{id}', [NotificationController::class, 'show']);


// route pour les Communes
Route::get('/communes', [CommuneController::class, 'index']);
Route::get('/communes/{id}', [CommuneController::class, 'show']);
Route::post('/communes', [CommuneController::class, 'store']);
Route::put('/communes/{id}', [CommuneController::class, 'update']);
Route::delete('/communes/{id}', [CommuneController::class, 'destroy']);


//route pour les roles
Route::get('/roles', [RoleController::class, 'index']);
Route::get('/roles/{id}', [RoleController::class, 'show']);
Route::post('/roles', [RoleController::class, 'store']);
Route::put('/roles/{id}', [RoleController::class, 'update']);
Route::delete('/roles/{id}', [RoleController::class, 'destroy']);
// route pour les postes
Route::get('/postes', [PosteController::class, 'index']);
Route::get('/postes/{id}', [PosteController::class, 'show']);


// route media
Route::post('/medias', [MediaController::class, 'store']);

//route pour les parents
Route::get('/parents', [ParentsController::class, 'index']);
Route::get('/parents/{id}', [ParentsController::class, 'show']);
//route pour repetiteur
Route::get('/repetiteurs', [RepetiteurController::class, 'index']);
Route::get('/repetiteurs/{id}', [RepetiteurController::class, 'show']);

// Route::middleware('auth:sanctum')->get('/user', function (Request $request) {
//     return $request->user();
// });
Route::middleware(['auth:sanctum', 'json-response'])->group(function () {
    Route::get("logout", [AuthController::class, 'logout']);
    Route::get('/profile', function(Request $request) {
        $user= auth()->user();
        // Récupérer les détails du rôle associé à l'utilisateur
    $roleDetails = Role::find($user->role_id);

    // Ajouter les détails du rôle à l'objet utilisateur
    $user->role = $roleDetails;

    return response()->json($user);
    });
    // routes pour les demandes
    Route::post('/demandes', [DemandeController::class, 'store']);
    Route::put('/demandes/{id}', [DemandeController::class, 'update']);
    Route::delete('/demandes/{id}',[DemandeController::class, 'destroy']);

    //routes pour les parents
    Route::post('/parents', [ParentsController::class, 'store']);
    Route::put('/parents/{id}', [ParentsController::class, 'update']);
    Route::delete('/parents/{id}', [ParentsController::class, 'destroy']);

    //routes pour les enfants
    Route::post('/enfants', [EnfantsController::class, 'store']);
Route::put('/enfants/{id}', [EnfantsController::class, 'update']);
Route::delete('/enfants/{id}',[EnfantsController::class, 'destroy']);

    //route pour les répétiteur
    Route::post('/repetiteurs', [RepetiteurController::class, 'store']);
    Route::put('/repetiteurs/{id}', [RepetiteurController::class, 'update']);
    Route::delete('/repetiteurs/{id}', [RepetiteurController::class, 'destroy']);

    //routes repetiteur matiere classe
    Route::post('/repetiteurmcs', [RepetiteurMatiereClasseController::class, 'store']);
Route::put('/repetiteurmcs/{id}', [RepetiteurMatiereClasseController::class, 'update']);
Route::delete('/repetiteurmcs/{id}',[RepetiteurMatiereClasseController::class, 'destroy']);


// Routes pour les payements
Route::post('/payements', [PayementController::class, 'store']);
Route::put('/payements/{id}', [PayementController::class, 'update']);
Route::delete('/payements/{id}',[PayementController::class, 'destroy']);

//Routes pour les postes
Route::post('/postes', [PosteController::class, 'store']);
Route::put('/postes/{id}', [PosteController::class, 'update']);
Route::delete('/postes/{id}', [PosteController::class, 'destroy']);

// Routes pour les notifications
Route::post('/notifications', [NotificationController::class, 'store']);
Route::put('/notifications/{id}', [NotificationController::class, 'update']);
Route::delete('/notifications/{id}', [NotificationController::class, 'destroy']);
});
