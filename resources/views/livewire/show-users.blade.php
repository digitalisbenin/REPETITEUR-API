<div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white p-4">
    ...
    <label for="table-search" class="block mb-2 text-3xl font-medium text-gray-900 dark:text-white">Liste des utilisateurs</label>
            ...
            @foreach ($users as $user)
                <tr
                    ...
                    {{--  <td class="flex items-center px-6 py-4 space-x-3">
                        ...
                        <a href="#" wire:click="delete({{ $user }})" wire:loading.attr="disabled"
    ...
    <x-dialog-modal wire:model="showEditModal" maxWidth="2xl">
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <x-input type="email" class="mt-1 block w-full text-xl" placeholder="{{ __('Email') }}"
                    x-ref="editing.email" wire:model.defer="editing.email"  />

                <x-input-error for="editing.email" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-input type="text" class="mt-1 block w-full text-xl" placeholder="{{ __('Password') }}"
                    x-ref="editing.password" wire:model.defer="editing.password"  />

                <x-input-error for="editing.password" class="mt-2" />
            </div>
            <div class="mt-4">

                <label for="editing.role_id"
                    class="block mb-2 text-xl font-medium text-gray-900 dark:text-white">Role</label>
                <select id="editing.role_id" wire:model.defer="editing.role_id"
