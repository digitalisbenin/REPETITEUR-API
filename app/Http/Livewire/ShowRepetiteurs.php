...
class ShowRepetiteurs extends Component
{
    ...
    protected $listeners = ['editRepetiteur' => 'showRepDetails'];
...
// }
    ...
    public function edite($repetiteurId)
    {
        // Émettre un événement pour indiquer que l'édition est demandée
        $this->emit('editRepetiteur', $repetiteurId);
        $this->dispatchBrowserEvent('console.log', ['message' => 'Event emitted']);
    }

    public function showRepDetails($repetiteurId)
{

    $repetiteurDetails = $repetiteurId;
    dd($repetiteurDetails);
    $this->emit('editRepetiteur', $repetiteurDetails);
}
    ...
    {
        $repetiteur = Repetiteur::latest('created_at')->get();
